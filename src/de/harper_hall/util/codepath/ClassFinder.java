 ...
 * ClassFinder.java
 */
package de.harper_hall.util.codepath;
...
/**
 * This utility class was based originally on <a href="http://www.velocityreviews.com/forums/member.php&u=47838">
 * Daniel Le Berre</a>'s <code>RTSI</code> class. This class can be called in different modes, but the principal 
 * use is to determine what subclasses/implementations of a given class/interface exist in the current runtime 
 * environment.
 * 
 * <a href=http://www.velocityreviews.com/forums/member.php?u=47838"Y>Elliott Wade</a> made some enhancements, 
 * Lutz Behnke just did a little code clean up.
 * 
 * @author Daniel Le Berre, Elliott Wade, Lutz Behnke
 */
public class ClassFinder {
  private static final String CLASS_FILE_NAME_EXTENSION = ".class";

  private static final int CLASS_FILE_NAME_EXTENSION_LEN = 6;
    ...
    synchronized (classpathLocations) {
      synchronized (results) {
        try {
          ...
          //
          if (fqcn.startsWith(".") || fqcn.endsWith(".")) { return new Vector<Class<?>>(); }
      ...
      for (int loc = 0; loc < knownLocations.size(); loc++) {
        ...
        if (directory.exists()) {
          ...
          for (int i = 0; i < files.length; i++) {
            ...
            if (files[i].endsWith(CLASS_FILE_NAME_EXTENSION)) {
              // removes the .class extension
              String classname = files[i].substring(0, files[i].length() - CLASS_FILE_NAME_EXTENSION_LEN);
          ...
          try {
            ...
            while (e.hasMoreElements()) {
              ...
              if (!entry.isDirectory() && entryname.endsWith(CLASS_FILE_NAME_EXTENSION)) {
                String classname = entryname.substring(0, entryname.length() - CLASS_FILE_NAME_EXTENSION_LEN);
                if (classname.startsWith("/")) {
                  classname = classname.substring(1);  // strip leading slash
                }
   ...
   * 
   * @param args fully qualified name of the super-class or interface as first element of array
   */
  public static void main(String[] args) {
    args = new String[] {"org.rlcommunity.rlglue.codec.AgentInterface"};
