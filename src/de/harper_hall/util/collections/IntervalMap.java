 ...
 * 
 * This interface extends the Map interface, so that a keys which are added together with the value define the border of
 * an interval, rather than an specific value.
 * 
 * The method get(K key) will return the val for the smallest key that is greater than or equeal to key.
 * 
...
public interface IntervalMap<K extends Comparable<K>, V> extends Map {
  ...
  /**
   * Use the last element for any values beyond the last entry. the method <tt>get(K key)</tt> will return null
   * otherwise.
   * 
   * @param lastForBexon
